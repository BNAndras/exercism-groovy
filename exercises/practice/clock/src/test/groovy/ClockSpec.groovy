...
class ClockSpec extends Specification {
    ...
    @Unroll
    def "Create a new clock with an initial time #label"() {
        given:
        Clock clock  = new Clock(hour, minute)
        expect:
        clock.toString() == expected
        ...
        "negative hour and minutes roll over"               | -25   | -160      || "20:20"
        "negative hour and minutes roll over continuously"  | -121  | -5810     || "22:10"
    }

    @Unroll
    @Ignore
    def "Add minutes - #label"() {
        given:
        Clock clock = new Clock(hour, minute)
        clock.add(value)
        expect:
        ...
        where:
        label                                       | hour  | minute    | value || expected
    ...
    @Unroll
    @Ignore
    def "Subtract minutes - #label"() {
        given:
        Clock clock = new Clock(hour, minute)
        clock.subtract(value)
        expect:
        clock.toString() == expected
    ...
    @Unroll
    @Ignore
    def "Compare two clocks for equality - #label"() {
        given:
        Clock clock1 = new Clock(hour1, minute1)
        Clock clock2 = new Clock(hour2, minute2)
        expect:
        clock1.equals(clock2) == expected

        where:
        label                                                   | hour1 | minute1   | hour2 | minute2  || expected
        ...
        "clocks with negative minute"                           | 2     | 40        | 2     | -20      || true
